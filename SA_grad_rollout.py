...
class SelfAttentionGradRollout:
    def __init__(self, model, attention_layer_name='softmax',
                 discard_ratio=0.9):
        self.model = model
    ...
    def __call__(self, prev_img, img, category_index):
        self.model.zero_grad()
        output, _ = self.model(prev_img, img)
        category_mask = torch.zeros(output.size())
        category_mask[:, category_index] = 1
        loss = (output * category_mask.cuda()).sum()
        loss.backward()
