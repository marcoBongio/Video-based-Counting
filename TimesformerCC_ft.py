...
from TimeSformer.timesformer.models import MODEL_REGISTRY
from variables import HEIGHT_TS, PATCH_SIZE_TS, NUM_FRAMES, EMBED_DIM, DEPTH_TS, NUM_HEADS, IN_CHANS, WIDTH_TS
...
class Block(nn.Module):
    ...
    def forward(self, x, B, T, W):
        num_spatial_tokens = (x.size(1) - 10) // T
        ...
        if self.attention_type in ['space_only', 'joint_space_time']:
            ...
            xt = x[:, 10:, :] + res_temporal

            ## Spatial
            init_flows_token = x[:, :10, :]
            flows_token = init_flows_token.repeat(1, T, 1, 1)
            flows_token = rearrange(flows_token, 'b t fl m -> (b t) fl m', b=B, t=T)
            xs = xt
            xs = rearrange(xs, 'b (h w t) m -> (b t) (h w) m', b=B, h=H, w=W, t=T)
            xs = torch.cat((flows_token, xs), 1)
            ...
            ## Mlp
            x = torch.cat((init_flows_token, x), 1) + torch.cat((flows_token, res), 1)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, height=224, width=224, patch_size=16, in_chans=3, embed_dim=768):
        super().__init__()
        patch_size = to_2tuple(patch_size)
        num_patches = (height // patch_size[1]) * (width // patch_size[0])
        self.height = height
        self.width = width
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, height=224, width=224, patch_size=16, in_chans=IN_CHANS, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embed = PatchEmbed(
            height=height, width=width, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
        num_patches = self.patch_embed.num_patches

        ## Positional Embeddings
        self.flows_token = nn.Parameter(torch.zeros(1, 10, embed_dim))
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches + 10, embed_dim))
        ...
        # output layer
        self.to_out = nn.Linear(embed_dim, WIDTH_TS * HEIGHT_TS)

        trunc_normal_(self.pos_embed, std=.02)
        trunc_normal_(self.flows_token, std=.02)
    ...
    def no_weight_decay(self):
        return {'pos_embed', 'flows_token', 'time_embed'}
    ...
    def forward_features(self, x):
        ...
        flows_tokens = self.flows_token.expand(x.size(0), -1, -1)
        x = torch.cat((flows_tokens, x), dim=1)

        ## resizing the positional embeddings in case they don't match the input at inference
        if x.size(1) != self.pos_embed.size(1):
            pos_embed = self.pos_embed
            flows_pos_embed = pos_embed[0, 0, :].unsqueeze(0).unsqueeze(1)
            ...
            new_pos_embed = torch.cat((flows_pos_embed, new_pos_embed), 1)
            x = x + new_pos_embed
        else:
            x = x + self.pos_embed
        x = self.pos_drop(x)
...
class TimeSformer(nn.Module):
    def __init__(self, height=HEIGHT_TS, width=WIDTH_TS, patch_size=PATCH_SIZE_TS, num_classes=0, num_frames=NUM_FRAMES,
        ...
        self.model = VisionTransformer(height=HEIGHT_TS, width=WIDTH_TS, num_classes=num_classes, patch_size=patch_size,
                                       embed_dim=EMBED_DIM, depth=DEPTH_TS, num_heads=NUM_HEADS, mlp_ratio=4,
                                       qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0.,
        ...
        if self.pretrained:
            load_pretrained(self.model, num_classes=self.model.num_classes, in_chans=kwargs.get('in_chans', 3),
                            filter_fn=_conv_filter, height=HEIGHT_TS, width=WIDTH_TS, num_frames=num_frames,
