...
import h5py
import cv2
from variables import PATCH_SIZE,HEIGHT,WIDTH
...
def load_data(img_path,train = True):
    ...
    post_img = post_img.resize((HEIGHT,WIDTH))

    gt_file = h5py.File(gt_path)
    target = np.asarray(gt_file['density'])
    gt_file.close()
    target = cv2.resize(target,(int(target.shape[1]/PATCH_SIZE),int(target.shape[0]/PATCH_SIZE)),interpolation = cv2.INTER_CUBIC)*64

    prev_gt_file = h5py.File(prev_gt_path)
    prev_target = np.asarray(prev_gt_file['density'])
    prev_gt_file.close()
    prev_target = cv2.resize(prev_target,(int(prev_target.shape[1]/PATCH_SIZE),int(prev_target.shape[0]/PATCH_SIZE)),interpolation = cv2.INTER_CUBIC)*64

    post_gt_file = h5py.File(post_gt_path)
    post_target = np.asarray(post_gt_file['density'])
    post_gt_file.close()
    post_target = cv2.resize(post_target,(int(post_target.shape[1]/PATCH_SIZE),int(post_target.shape[0]/PATCH_SIZE)),interpolation = cv2.INTER_CUBIC)*64
